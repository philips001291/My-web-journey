...
app.get("/filter", (req, res) => {
  const jokeType = parseInt(req.params.jokeType);
  const foundJoke = jokes.find((joke) => joke.jokeType === jokeType);
  res.json(foundJoke);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res
      .status(404)
      .json({ error: `Jokes with id: ${id} not found. No jokes were deleted` });
  }
});
